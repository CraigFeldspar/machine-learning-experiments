...
def dilate(ary, N, iterations):
    ...
    return (dilated_image * 255).astype("uint8")
...
def process_image(path, out_path):
    ...
    borders.sort(key=lambda p: (p[3] - p[1]) * (p[4] - p[2]))
    ...
    # im.show()
    text_im = orig_im[crop[1]:crop[3], crop[0]:crop[2]]
...
if __name__ == '__main__':
    ...
    for path in files:
        ...
        print(pytesseract.image_to_string(Image.open(out_path), lang='fra'))
...
out_path = path.replace('.jpg', '.crop.png')
#process_image(path, out_path)
print(pytesseract.image_to_string(Image.open(path), lang='fra', config='--psm 3'))
